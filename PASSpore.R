...
ui <- dashboardPage(skin = "purple",
                    ...
                    dashboardHeader(title = "PASSpore"
                                    
                    ),
                    #========= Sidebar ===========
                    dashboardSidebar(
                      sidebarMenu(id = "sidebar",
                                  menuItem("  .Home", tabName = "home", icon = icon("home", style = "color: #9d52ff")),
                                  ...
                                  conditionalPanel(condition = "input.sidebar == 'single'",
                                                   ...
                                                   numericInput("seuilPercU", "Cut-Off : minimal percentage of U added :", value = 60),
                                  ...
                                  conditionalPanel(condition = "input.sidebar == 'multiple'",
                                                   numericInput("seuilPercUMultiple", "Cut-Off : minimal percentage of U added :", value = 60),
                    ...
                    dashboardBody(
                      tabItems(
                        tabItem(tabName = "home", 
                            h1(strong("Welcome"), style = 'color:#9d52ff;font-size:80px;', align = "center"),
                            em("this is the intership project of ", span("Martin Schoenstein", style = "color:#9d52ff")),
                            p(""),
                            br(),
                            br(),
                            p("PASSpore is a Shiny application for FlepSeq 2 runs ", span(strong("visualisation of 3' mRNA."))), 
                            p("You can access to plots for ", span(strong("one AGI", icon("play", style = "color: #9d52ff"))), "or for ", span(strong("all of them together")), icon("forward", style = "color: #9d52ff")),
                            p("Compare your poly(A) distribution with the", span(strong("PASS score", icon("code", style = "color: #9d52ff"))), "method and obtain a score per genes."), 
                            p(""),
                            br(),
                            selectInput("run", "Choose your Run :", listdir),
                            actionButton("buttonHome", label = "Confirm this run", icon = icon("check", style = "color: #1dcc1d")),
                        ...
                        tabItem(tabName = "single",
                                ...
                                #h3("Poly(A) tail :"),
                                #p(""),
                                box(title = "Poly(A) tail length distribution :", status = "primary", solidHeader = TRUE, width = 12, plotOutput("polyaDistributionSingle"), radioButtons("plotType", "Change plot", c("Density plot", "Cumulativ plot", "Histogram plot", "Boxplot"), selected = "Density plot"), downloadButton("downloadPolyaDistribPlot", "Download this plot")),
                                #p(""),
                                box(title = "Poly(A) tail length distribution in uridylate mRNA", status = "primary", solidHeader = TRUE, width = 12, plotOutput("polyaDistributionUrdiylSingle"), radioButtons("plotTypeUridyl", "Change plot", c("Density plot", "Cumulativ plot", "Histogram plot", "Boxplot"), selected = "Density plot"), downloadButton("downloadPolyaDistributionUridylPlot", "Download this plot")),
                                p(".")
                        ),
                        ...
                        tabItem(tabName = "multiple",
                                ...
                                box(title = "Poly(A) tail length distribution :", status = "primary", solidHeader = TRUE, width = 12, plotOutput("polyaDistributionMultiple"), radioButtons("plotTypeMultiple", "Change plot", c("Density plot", "Cumulativ plot", "Histogram plot", "Boxplot"), selected = "Density plot"), downloadButton("downloadPolyaDistribMultiplePlot", "Download this plot")),
                                box(title = "Poly(A) tail length distribution in uridylate mRNA", status = "primary", solidHeader = TRUE, width = 12, plotOutput("polyaDistributionUridylMultiple"), radioButtons("plotTypeUridylMultiple", "Change plot", c("Density plot", "Cumulativ plot", "Histogram plot", "Boxplot"), selected = "Density plot"), downloadButton("downloadPolyaDistributionUridylMultiplePlot", "Download this plot")),
                                p(".")
                        ),
                        ...
                        tabItem(tabName = "pass",
                                h1("PASS Score"),
                                p(""),
                                box(title = "Cumulativ poly(A) tail length plot :", status = "primary", solidHeader = TRUE, width = 12, plotOutput("cumulativPassPlot")),
                                numericInput("passMin", "Lower poly(A) tail length limit for PASS Score", value = 10, min = 10, max = 200),
                                numericInput("passMax", "Upper poly(A) tail length limit for PASS Score", value = 200, min = 10, max = 200),
                                #h5("It's important to choose the right limits for the PASS Score.", style = "color : #7c7f80"),
                                actionButton("buttonValidationPass2", "  Calculate the PASS Score", icon = icon("volleyball", style = "color: #9d52ff")),
                                h4(""),
                                textOutput("meanPass"),
                                h4(""),
                                dataTableOutput("tablePASSscore")
                        )
                      )
                    ) 
)
...
server <- function(input, output) { 
  ...
  output$IBMP = renderUI({
    img(src = "ibmp.png", height = '100px')
  })
  ...
  listCsvReact = eventReactive(input$buttonHome, {                #liste réatcive des différents fichier csv pour le run choisi
    listCsv = list.files(paste0(PATHo, "/", input$run, "/4_Tail"), full.names = FALSE, pattern = "csv$")
  })
  
  listBarcodeReact = eventReactive(input$buttonHome, {            #liste réactive des différents barcodes présents
    listBarcode = c()
  ...
  bindCsvFilesReact = eventReactive(input$buttonHome, {           #fusion en un tableau réactif des fichiers csv et ajout d'une ligne barcode (valeure 1 à x en fonction de l'ordre alphabétique)
    bindCsvFiles = rbindlist(lapply(paste0(paste0(PATHo, "/", input$run, "/4_Tail"), "/", listCsvReact()), fread), idcol = "barcode")
  })
  
  observeEvent(input$buttonHome, {
    shinyalert(title = "Processing", text = "can be a bit long, depending on files sizes", type = "warning", closeOnEsc = FALSE, closeOnClickOutside = FALSE, showCancelButton = FALSE, showConfirmButton = FALSE)
    bindCsvFilesReact()
    shinyalert(title = "It's Done !", type = "success", closeOnEsc = TRUE, closeOnClickOutside = TRUE, showCancelButton = FALSE, showConfirmButton = TRUE, immediate = TRUE)
    output$uridylationPercentageMultiple = renderPlot(plotNull)
    output$uridylationNumberMultiple = renderPlot(plotNull)
    output$polyaDistributionMultiple = renderPlot(plotNull)
    output$polyaDistributionUridylMultiple = renderPlot(plotNull)
    output$tableStatReadsMultiple = renderTable(tableNull)
  })
  
  #- - - - -
  
  observeEvent(input$buttonHome, {          #mise à jour du choix de gène en fonction de ceux présents dans le fichier fusionné
    updateSelectInput(inputId = "geneSingle", choices = read_tsv(paste0(PATHo, "/", input$run, "/listgene.csv"))$gene)
  })
  ...
  dfUridylMultipleReact = eventReactive(input$buttonValidationMultiple, {      #tableau modifié en ajoutant une colonne pour différencier les lignes uridylés des autres (selon le pourcentage d'input) et mais on garde tout le gène cette fois ci, c'est ce tableau qu'on utilisera après pour le multiple, ne se met à jour que lors de la validation des inputs
    dfUridylMultiple = bindCsvFilesReact()%>%mutate(uridyl = if_else(add_tail_pct_T>input$seuilPercUMultiple, "U", "/"))%>%mutate(x = ".")
  })
  
  # - - - - -
  
  barcodeCorrespondancePassReact = reactive({
    listBarcodeCorrespondancePass = list.files(paste0(PATHo, "/", input$run),full.names = TRUE, pattern = "^barcode_correspondance")
    ...
    if (length(barcodeCorrespondancePass) == 1){         #si le tableau n'a que 1 colonne ça veut dire que dans le fichier de base barcode_correspondance on a le barcore et le genotype dans la même colonne donc on sépare
      barcodeCorrespondancePass = barcodeCorrespondancePass%>%separate(X1, sep = " ", c("Barcode", "Genotype"))
    }
  ...
  PassbindCsvReact = reactive({
    ...
    RightPasslistCsv = c()
    PasslistCsv = list.files(paste0(PATHo, "/", input$run, "/4_Tail"), full.names = FALSE, pattern = "csv$")
    ...
    PassbindCsv = rbindlist(lapply(paste0(paste0(PATHo, "/", input$run, "/4_Tail"), "/", RightPasslistCsv), fread), idcol = "barcode")
    return(PassbindCsv)
  })
  ...
  observeEvent(input$buttonValidationSingle, {     #tout ce qui se passe qu'une fois que la séléctionner du run et gène est validé
    ...
    listBarcodeCorrespondance = list.files(paste0(PATHo, "/", input$run),full.names = TRUE, pattern = "^barcode_correspondance")
    ...
    tableStatReads = barcodeCorrespondance%>%mutate(NumberOfReads = ReadsNumber, NumberOfReadsU = ReadsUridylNumber)
    if (length(tableStatReads) == 3){         #si le tableau n'a que trois colonne ça veut dire que dans le fichier de base barcode_correspondance on a le barcore et le genotype dans la même colonne donc on sépare
    ...
    observe({                                                 #le plot dépend de l'input présent sous le graphique, on affiche le densité par défault
      ...
      if (input$plotType == "Histogram plot"){
        output$polyaDistributionSingle = renderPlot({
          print(plotPolyaDistributionSingle4React())
        })
      }
    })
    ...
    observe({                                               #pareil
      ...
      if (input$plotTypeUridyl == "Boxplot"){
        output$polyaDistributionUrdiylSingle = renderPlot({
      ...
      if (input$plotTypeUridyl == "Histogram plot"){
        output$polyaDistributionUrdiylSingle = renderPlot({
          print(plotPolyaDistributionUridylSingle4React())
        })
      }
    })
  })
  ...
  output$downloadUrdidylPercPlot = downloadHandler(        #Télechargement du plot en image
    filename = function(){
      paste0("plotPercentageUridylation-",input$run, "-", input$geneSingle, ".png")
    },
  ...
  output$downloadUrdidylNumberPlot = downloadHandler(        #Télechargement du plot en image
    filename = function(){
      paste0("plotNumberUridyl-",input$run, "-", input$geneSingle, ".png")
    },
    content = function(file){
  ...
  plotPolyaDistributionSingle4React = reactive({
    plotPolyaDistributionSingle = ggplot(dfUridylReact(), aes(x = polya_length, group = barcode, color = as.character(barcode), alpha = 1, fill = as.character(barcode))) + 
      geom_density() + 
      xlim(10, 200) +
      geom_histogram(aes(y = ..density..), colour = "black", fill = "white", alpha = 0, binwidth = 3, size = 0.3) + 
      facet_wrap(vars(barcode)) + guides(color = FALSE, alpha = FALSE) + 
      theme(legend.title = element_text(size = 14), legend.text = element_text(size = 10), legend.position = "bottom") + 
      xlab("PolyA tail length") + ylab("Density of distribution") + 
      scale_fill_hue(name = "", labels = paste(listBarcodeReact())) 
  })
  ...
  output$downloadPolyaDistribPlot = downloadHandler(           #Télechargement du plot en image, le plot téléchargé dépend de celui affiché donc de la valeur de l'input
    filename = function(){
      paste0("plotDistributionPolya-",input$run, "-", input$geneSingle, ".png")
    },
    content = function(file){
      if (input$plotType == "Density plot"){
      ...
      else if (input$plotType == "Histogram plot"){
        ggsave(file, plotPolyaDistributionSingle4React(), width = 12)
      }
      else {
  ...
  plotPolyaDistributionUridylSingle4React = reactive({
    plotPolyaDistributionSingle = ggplot(dfUridylReact()%>%filter(uridyl == "U"), aes(x = polya_length, group = barcode, color = as.character(barcode), alpha = 1, fill = as.character(barcode))) + 
      geom_density() + 
      xlim(10, 200) +
      geom_histogram(aes(y = ..density..), colour = "black", fill = "white", alpha = 0, binwidth = 3, size = 0.3) + 
      facet_wrap(vars(barcode)) + guides(color = FALSE, alpha = FALSE) + 
  ...
  observeEvent(input$buttonValidationMultiple, {     #tout ce qui se passe qu'une fois que la séléctionner du run est validé
    shinyalert(title = "Processing", text = "can be a bit long, depending on files sizes", type = "warning", closeOnEsc = FALSE, closeOnClickOutside = FALSE, showCancelButton = FALSE, showConfirmButton = TRUE)
    # ---------->Tableau récap pour le run (barcode et génotype, nombre de reads)
    
    listBarcodeCorrespondanceMultiple = list.files(paste0(PATHo, "/", input$run),full.names = TRUE, pattern = "^barcode_correspondance")
    ...
    tableStatReadsMultiple = barcodeCorrespondanceMultiple%>%mutate(NumberOfReads = ReadsNumberMultiple, NumberOfReadsU = ReadsUridylNumberMultiple)
    if (length(tableStatReadsMultiple) == 3){         #si le tableau n'a que trois colonne ça veut dire que dans le fichier de base barcode_correspondance on a le barcore et le genotype dans la même colonne donc on sépare
    ...
    observe({                                                 #le plot dépend de l'input présent sous le graphique, on affiche le densité par défault
      ...
      if (input$plotTypeMultiple == "Boxplot"){
        output$polyaDistributionMultiple = renderPlot({
      ...
      if (input$plotTypeMultiple == "Histogram plot"){
        output$polyaDistributionMultiple = renderPlot({
          print(plotPolyaDistributionMultiple4React())
        })
      }
    })
    ...
    observe({                                               #pareil
      ...
      if (input$plotTypeUridylMultiple == "Boxplot"){
        output$polyaDistributionUridylMultiple = renderPlot({
      ...
      if (input$plotTypeUridylMultiple == "Histogram plot"){
        output$polyaDistributionUridylMultiple = renderPlot({
          print(plotPolyaDistributionUridylMultiple4React())
        })
      }
    })
  })
  ...
  output$downloadUridylPercMultiplePlot = downloadHandler(        #Télechargement du plot en image
    filename = function(){
      paste0("plotPercentageUridylation-",input$run, "-", ".png")
    },
    content = function(file){
  ...
  output$downloadUridylNumberMultiplePlot = downloadHandler(        #Télechargement du plot en image
    filename = function(){
      paste0("plotNumberUridyl-",input$run, "-", ".png")
    },
    content = function(file){
  ...
  plotPolyaDistributionMultiple4React = reactive({
    plotPolyaDistributionSingle = ggplot(dfUridylMultipleReact(), aes(x = polya_length, group = barcode, color = as.character(barcode), alpha = 1, fill = as.character(barcode))) + 
      geom_density() + 
      xlim(10, 200) +
      geom_histogram(aes(y = ..density..), colour = "black", fill = "white", alpha = 0, binwidth = 3, size = 0.3) + 
      facet_wrap(vars(barcode)) + guides(color = FALSE, alpha = FALSE) + 
  ...
  output$downloadPolyaDistribMultiplePlot = downloadHandler(           #Télechargement du plot en image, le plot téléchargé dépend de celui affiché donc de la valeur de l'input
    filename = function(){
      paste0("plotDistributionPolya-",input$run, "-", ".png")
    },
    content = function(file){
      if (input$plotTypeMultiple == "Density plot"){
      ...
      else if (input$plotTypeMultiple == "Histogram plot"){
        ggsave(file, plotPolyaDistributionMultiple4React(), width = 12)}
      else {
  ...
  plotPolyaDistributionUridylMultiple4React = reactive({
    plotPolyaDistributionSingle = ggplot(dfUridylMultipleReact()%>%filter(uridyl == "U"), aes(x = polya_length, group = barcode, color = as.character(barcode), alpha = 1, fill = as.character(barcode))) + 
      geom_density() + 
      xlim(10, 200) +
      geom_histogram(aes(y = ..density..), colour = "black", fill = "white", alpha = 0, binwidth = 3, size = 0.3) + 
      facet_wrap(vars(barcode)) + guides(color = FALSE, alpha = FALSE) + 
  ...
  observeEvent(input$buttonValidationPass2, {
    ...
    output$meanPass = renderText(paste("Mean Pass score :", round(sum(dataframePASS[,2]) / nrow(dataframePASS), 2)))
    output$tablePASSscore = renderDataTable(dataframePASS %>% arrange(desc(PASScore)))
    write_csv2(dataframePASS %>% arrange(desc(PASScore)), paste0("/Users/jroignant/Desktop/Stage_Martin/mar2024-main/DocMartin/resultat", "/PASS", input$run ,".csv"))
  })
